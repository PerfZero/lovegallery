      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <DSLabel className="flex items-center gap-2">
            <Ruler size={12} />{" "}
            {isPetFurniture ? "ГАБАРИТЫ (СМ)" : "РАЗМЕР (СМ)"}
          </DSLabel>
        </div>
        ...
        {isPetFurniture && (
          <button
            ...
            className="w-full text-left flex items-start gap-2 bg-accent/5 p-3 rounded-sm hover:bg-accent/10 transition-colors"
          >
            <Info size={12} className="text-accent mt-0.5" />
            <DSText size="sm" className="text-accent/80 italic leading-snug">
              Не можете определиться с размером? Обратитесь к нашему эксперту за
              консультацией.
            </DSText>
          </button>
        )}
            ...
            <div className="flex flex-wrap gap-2">
              {artwork.options?.finishes?.map((finish) => (
                <button
                  ...
                  className={`px-4 py-2 text-[10px] tracking-widest uppercase transition-all border ${
              ...
              <div className="flex gap-2">
                {binaryOptions.map((opt) => (
                  <button
                    ...
                    className={`flex-1 py-2 text-[10px] tracking-widest border transition-all ${
                      hasFrame === opt
                    ...
                    <Popover>
                      ...
                      >
                        <DSText size="sm" className="leading-relaxed">
                          Музейное стекло с антибликовым покрытием, защищает от
                          ультрафиолета на 70-99% и обеспечивает максимальную
                          прозрачность.
                        </DSText>
                      </PopoverContent>
                    </Popover>
            ...
            <div className="flex gap-2">
              {productTypes.map((type) => (
                <button
                  ...
                  className={`flex-1 py-4 text-[10px] tracking-[0.2em] font-semibold border transition-all ${
                    productType === type
                      ? "border-accent bg-accent/5 text-accent"
                      : "border-border/10 text-muted-foreground hover:border-border"
                  }`}
