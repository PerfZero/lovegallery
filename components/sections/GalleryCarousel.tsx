...
import { galleryImagesData, type ResponsiveSize, type ZIndexLevel, type RoundedVariant } from "@/data/gallery-images";
...
const GalleryCarousel = () => {
    ...
    const carouselItems = useMemo(() => {
        return artworks.map((artwork, index) => {
            ...
            return {
                ...
                categoryLabel: categoryThemes[artwork.category]?.title || artwork.category,
    ...
    return (
        <section
            className={`h-[40vh] md:h-[50vh] flex items-start select-none pb-4 md:pb-6 w-full transition-all duration-700 pointer-events-auto ${isPreloading ? 'overflow-visible' : 'overflow-hidden'
                }`}
        >
            <div className="relative flex w-full">
                <div
                    className={`flex gap-0 animate-marquee whitespace-nowrap hover:[animation-play-state:paused] ${isPreloading ? 'paused' : ''
                        }`}
                >
...
const GalleryCard = forwardRef<HTMLDivElement, GalleryCardProps>(
    ({ item }, ref) => {
        const { artwork, href, categoryLabel, widthPx, heightPx, marginTopPx, marginLeftPx, zIndex, rounded } = item;

        // Compute aspect ratio multiplier
        const ratioMultiplier = artwork.aspectRatio === 'portrait' ? 1.33 : artwork.aspectRatio === 'landscape' ? 0.75 : 1;
        ...
        return (
            <div
                ref={ref}
                style={style}
                className={`inline-block transition-transform duration-700 hover:scale-[1.02] relative group mt-[var(--mt-mob)] md:mt-[var(--mt-desk)] ml-[var(--ml-mob)] md:ml-[var(--ml-desk)] will-change-transform z-${zIndex}`}
            >
                <Link
                    href={href}
...
const GalleryCardOverlay = ({ title, category, price }: GalleryCardOverlayProps) => (
