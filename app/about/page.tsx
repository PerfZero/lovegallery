...
import { DSSection, DSContainer, DSLabel, DSText, DSHeading, DSDecorativeAsterisk } from '@/components/ui/design-system';
...
export default function AboutPage() {
    ...
    const [videoModal, setVideoModal] = useState<{ isOpen: boolean, src: string | null }>({ isOpen: false, src: null });
    ...
    return (
        <div className="min-h-screen bg-background text-foreground selection:bg-accent selection:text-white">
                    ...
                    <DSContainer size="narrow" className="text-center">
                        <Reveal direction="down">
                            ...
                            <div className="max-w-xl mx-auto border-t border-black/5 pt-12">
                                <DSText size="lg" className="italic font-display text-2xl text-muted-foreground/80 leading-relaxed">
                                    {hero.description}
                                </DSText>
                            </div>
                        </Reveal>
                    ...
                    <DSContainer size="wide">
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-px bg-black/5 p-px">
                            {categories.map((cat, index) => (
                                <Reveal key={index} delay={index * 100} className="bg-background group relative overflow-hidden aspect-[3/4]">
                                    <Image
                                        src={cat.image}
                                        alt={cat.title}
                                        fill
                            ...
                            <DSContainer>
                                <div className={`flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-16 md:gap-32`}>
                                    ...
                                    <div className="w-full md:w-1/2 relative">
                                        <Reveal direction={isEven ? "left" : "right"} className="relative z-10">
                                            <div
                                                ...
                                                onClick={() => item.video && setVideoModal({ isOpen: true, src: item.video })}
                                            >
                                                ...
                                                {item.video ? (
                                                    ...
                                                    <Image
                                                        src={item.image || `/images/gallery-${(index % 6) + 1}.webp`}
                                                        alt={item.title}
                                                        fill
                                        ...
                                        <div className={`absolute top-0 ${isEven ? '-right-12' : '-left-12'} z-0 select-none hidden md:block opacity-10`}>
                                        ...
                                        {/* Captions Detail */}
                                        <Reveal delay={400} className="absolute -bottom-8 -right-4 md:-right-12 bg-white/80 backdrop-blur-md p-6 md:p-10 shadow-2xl border border-black/5 max-w-[280px] hidden md:block z-20 transition-all duration-700 hover:bg-white/90">
                                            <div className="relative">
                                                {item.letter !== 'А' && item.letter !== 'Б' && (
                                                    <DSLabel className="mb-3 text-accent transition-colors">Принцип {item.letter}</DSLabel>
                                                )}
                                                <p className="font-display italic text-lg md:text-xl leading-snug text-foreground/80">
                                                    {item.letter === 'А' ? (
                                                        <>
                                                            ...
                                                            <Link href="/catalog/collections" className="text-accent underline decoration-accent/30 hover:decoration-accent transition-all">
                                                                ШАНТАРАМ
                                                            </Link>
                                                        </>
                                                    ) : item.letter === 'Б' ? (
                                        ...
                                        <Reveal delay={200} direction="up">
                                            <div className="space-y-12">
                                                <div className="flex items-center gap-6">
                                                    ...
                                                    <DSLabel className="text-accent">{item.title}</DSLabel>
                                                </div>

                                                <DSHeading level="h2" className="text-5xl md:text-7xl lg:text-8xl leading-[0.9] -ml-2">
                                                    {item.title.split(' ')[0]}
                                                    <br />
                                                    <span className="text-accent/80 italic text-4xl md:text-6xl">{item.title.split(' ').slice(1).join(' ')}</span>
                                                </DSHeading>

                                                <div className="space-y-8 pl-0 md:pl-12">
                                                    <DSText size="base" className="leading-loose font-light text-foreground/70">
                                                        {item.description}
                                                    </DSText>
                    ...
                    <DSContainer size="narrow" className="relative z-10 text-center">
                        <Reveal>
                            <div className="max-w-4xl mx-auto mb-16">
                                <DSHeading level="h2" className="text-3xl md:text-5xl lg:text-6xl text-white font-medium leading-snug drop-shadow-2xl mb-6 tracking-wide">
                                    Искусство требует.
                                </DSHeading>
                                <DSHeading level="h2" className="text-2xl md:text-4xl lg:text-5xl text-accent font-medium leading-snug tracking-wide">
            ...
            {/* Video Modal */}
            <Dialog open={videoModal.isOpen} onOpenChange={(open) => !open && setVideoModal(prev => ({ ...prev, isOpen: false }))}>
