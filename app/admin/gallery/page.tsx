...
import { Save, Upload, SlidersHorizontal, Trash2 } from "lucide-react";
    ...
    return (
        <div className="space-y-8 pb-20">
                        ...
                        <Card className="bg-white/60 backdrop-blur-sm border-border/40 shadow-sm overflow-hidden transition-all hover:shadow-md relative z-10">
                            <div className="aspect-[4/5] w-full bg-secondary/20 relative">
                                ...
                                <div className="absolute top-2 right-2">
                                    <Badge variant="secondary" className="bg-white/80 backdrop-blur-sm">Z-{parseInt(image.z.replace('z-', ''))}</Badge>
                                </div>
                            </div>
                            ...
                            <CardContent className="p-4 space-y-4">
                                ...
                                <div className="space-y-3 pt-2 border-t border-border/30">
                                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                                        <span className="flex items-center gap-1"><SlidersHorizontal size={12} /> Отступ сверху</span>
                                        <span>{image.marginTopPx.desktop}px</span>
                                    </div>
                                    <Slider defaultValue={[image.marginTopPx.desktop || 0]} max={200} step={10} className="w-full" />
                                </div>

                                <div className="space-y-3">
                                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                                        <span className="flex items-center gap-1"><SlidersHorizontal size={12} /> Отступ слева</span>
                                        <span>{image.marginLeftPx.desktop}px</span>
                                    </div>
                                    <Slider defaultValue={[Math.abs(image.marginLeftPx.desktop)]} max={100} step={5} className="w-full" />
                                </div>
            ...
            <div className="fixed bottom-8 right-8">
                <Button size="lg" className="rounded-full shadow-xl bg-foreground text-background hover:bg-foreground/90">
